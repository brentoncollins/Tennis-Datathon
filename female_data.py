...
def elo_lookup():
	...
	for key, value in item_list.items():
		if set(current_match_players[0].split(' ')).issubset(key.split(' ')):
			...
			if str(current_match_players[0].split(' ')[-1]) == str(key.split(' ')[-1]) or str(current_match_players[1].split(' ')[-2]) == str(key.split(' ')[-1]):
				player_1_data = value[0]
				player_1_name = key
	...
	if df.empty:
		...
		print (
			'{} and {} have not played before but the ranking suggests that {} should have a chance of {}&'.format(
				current_match_players[0], current_match_players[1],current_match_players[0],player_1_change))
...
def check_list():
	found = False
	for key, value in item_list.items():
		if set(current_match_players[0].split(' ')).issubset(key.lower().split(' ')):
			player_1_data = value[0]
			player_1_name = key
			found = True
		else:
			if set(current_match_players[0].split(' ')[-1]).issubset(key.lower().split(' ')):
				player_1_data = value[0]
				player_1_name = key
				found = True

		if set(current_match_players[1].split(' ')).issubset(key.lower().split(' ')):
			player_2_data = value[0]
			player_2_name = key
			found = True
		else:
			if str(current_match_players[1].split(' ')[-1]) == str(key.split(' ')[-1]) or str(current_match_players[1].lower().split(' ')[-2]) == str(key.lower().split(' ')[-1]):
				player_2_data = value[0]
				player_2_name = key
				found = True
		if found is False:
			print("Not found{} {}".format(key,current_match_players) )
			not_found.append(key)


df_players = pandas.read_csv('women_dummy_submission_file.csv', low_memory=False)
df_matches = pandas.read_csv('WTA_matches.csv', low_memory=False)
df_completed = pandas.read_csv('submission_women.csv', low_memory=False)
...
item_list['hugo dellien'] = [15900,0]#????
item_list['juan ignacio londero'] = [1550,0,0]#????
item_list['marco trungelliti'] = [1773,0,0]#????
item_list['adrian menendez maceiras'] = [1681,0,0]#????

# Women not in elo list

item_list['tamara zidansek'] = [1558,0,0]
item_list['anastasia potapova'] = [1696,0,0]
item_list['katie boulter'] = [1690,0,0]
item_list['olga danilovic'] = [1546,0,0]
item_list['anhelina kalinina'] = [1682,0,0]
item_list['mariana duque'] = [1680,0,0]
item_list['marta kostyuk'] = [1661,0,0]
item_list['caroline dolehide'] = [1698,0,0]
...
for index, row in df_players.iterrows():
	...
	current_match_players = (
		row['player_1'].lower().replace('-',' '),
		row['player_2'].lower().replace('-', ' '))

	print (df_completed.loc[index, 'player_1_win_probability'])
	#if float(df_completed.loc[index, 'player_1_win_probability']) < float(0.5):
	elo_lookup()
	df_completed.to_csv('submission_women.csv',  index=False)
	#else:
